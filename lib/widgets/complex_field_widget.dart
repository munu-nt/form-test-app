...
class ComplexFieldWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (children.isEmpty) {
      return Card(
        child: Padding(
          ...
          child: Text(
            "No sub-fields found for ${parentField.fieldName} (ID: ${parentField.fieldId})",
          ),
        ),
      );
    }
    ...
    if (formData[parentField.fieldId] is Map) {
      currentGroupData = Map<String, dynamic>.from(
        formData[parentField.fieldId],
      );
    }
    Map<String, dynamic> virtualData = {};
    for (var child in children) {
      String virtualId =
          "${parentField.fieldId}_${child.fieldName.replaceAll(RegExp(r'\s+'), '')}";
            ...
            ...virtualChildren.map((vField) {
              return DynamicFormField(
                ...
                onValueChanged: (id, val) {
                  ...
                  if (formData[parentField.fieldId] is Map) {
                    newGroupData = Map<String, dynamic>.from(
                      formData[parentField.fieldId],
                    );
                  }
